...
namespace Luggage
{
    public partial class inputinfo6 : Form
    {
        ...
        private void inputinfo6_Load(object sender, EventArgs e)
        {
        ...
        private void UpdateSummary()
        {
            ...
            label4.Text = totalPrice.ToString("C0", new CultureInfo("th-TH")); // Format to currency
        }
            ...
            using (MySqlConnection conn = databaseConnection())
            {
                ...
                WHERE 
                    ...
                    using (MySqlCommand checkCmd = new MySqlCommand(checkQuery, conn))
                    {
                        ...
                        checkCmd.Parameters.AddWithValue("@username", username_ii3);
                    ...
                    {
                        // อัพเดตข้อมูลในตาราง customer_info
                        string updateQuery = "UPDATE customer_info SET name = @name, email = @email, idenno = @idenno, address = @address, tel = @tel WHERE username = @username";
                        using (MySqlCommand updateCmd = new MySqlCommand(updateQuery, conn, transaction))
                        {
                            updateCmd.Parameters.AddWithValue("@name", cname.Text);
                            updateCmd.Parameters.AddWithValue("@email", cemail.Text);
                            updateCmd.Parameters.AddWithValue("@idenno", ciden.Text);
                            updateCmd.Parameters.AddWithValue("@address", caddress.Text);
                            updateCmd.Parameters.AddWithValue("@tel", ctel.Text);
                            updateCmd.Parameters.AddWithValue("@username", username_ii3);
                ...
                using (MySqlCommand deleteCmd = new MySqlCommand(deleteQuery, conn))
                {
                    ...
                    try
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            // เปิดฟอร์ม inputinfo2 และส่งค่า username
                            inputinfo2 inputinfo2Form = new inputinfo2();
                            inputinfo2Form.username_ii2 = username_ii3; // ส่งค่า username
                            inputinfo2Form.Show(); // แสดงฟอร์ม inputinfo2
